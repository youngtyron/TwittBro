         ...
         for (i = 0; i < Object.keys(context).length; i++){
           if (context[i].you_liked){
             $('.post-column').append("<li class='list-group-item posts' ><div class = 'one-post' id = 'current' name = '" + context[i].id+ "'>" + " <a href='" + context[i].link  +
                                        "'><p>" + context[i].avatar + " " + context[i].f_n + " " + context[i].l_n + "</p></a> <p class = 'post-text'>" +
                                        context[i].text + "</p><p class = 'post-date'>" +  context[i].date + "</p><p>" + context[i].images + "</p><i class='fas fa-heart' name = 'liking-from-news' id= '" +
            ...
            $('#current').append("<form action='' method='post' class='comment-sent-from-news' name='empty'><div class='hidden-comment-form' style = 'display: none;'><input name='csrfmiddlewaretoken' value ='" + token
                                ...
                                "<input value='submit' class='btn btn-dark add-comment' type='submit'></div></form>" + "<form action='' method='post' class='com-com-sent-from-news' name='empty'><div class='hidden-com-com-form' style = 'display: none;'><input name='csrfmiddlewaretoken' value ='" + token
                                + "'type='hidden'><p><label for='id_com_com-text'>Text:</label> <textarea name='com_com-text' maxlength='500' cols='2' rows='2' id='id_com_com-text' class='form-control'></textarea></p>" +
            ...
            if (context[i].delete_cross){
              $('#current').find('.post-text').before("<i class='fas fa-times' style ='position: absolute; right: 5%; left: 95%;' name ='delete-from-news'></i>")
            }
